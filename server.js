// ── FREEISLE RELAY ── No database, no storage, pure message relay ──
...
const server = http.createServer((req, res) => {
  ...
  res.end(JSON.stringify({
    ...
    note: 'No database. No storage. Messages pass through RAM only.'
  }))
})
...
function queue(id, data) {
  ...
  if (pending[id].length > 200) pending[id].shift()
}
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (msg.type === 'register') {
      ...
      if (users[id] && users[id] !== ws) {
        try { users[id].close() } catch(e) {}
      }
      users[id] = ws
      ws.send(JSON.stringify({ type: 'registered', userId: id, online: Object.keys(users).length }))
      // Flush pending messages
      if (pending[id]) {
        pending[id].forEach(m => { try { ws.send(JSON.stringify(m)) } catch(e) {} })
        delete pending[id]
      }
      if (friendReqs[id]) {
        friendReqs[id].forEach(r => { try { ws.send(JSON.stringify(r)) } catch(e) {} })
        delete friendReqs[id]
      }
    ...
    if (msg.type === 'friend_request') {
      ...
      const data = {
        type: 'msg', from: id, fromName: msg.fromName || id,
        to: msg.to, text: msg.text, id: msg.id,
        ts: msg.ts || Date.now(), msgType: msg.msgType || 'text',
      ...
      const data = {
        type: 'group_msg', from: id, fromName: msg.fromName || id,
        groupId: msg.groupId, groupName: msg.groupName || '',
        text: msg.text, id: msg.id, ts: msg.ts || Date.now()
      }
      ;(msg.members || []).forEach(uid => { if (uid !== id) { if (!to(uid, data)) queue(uid, data) } })
    }
    else if (msg.type === 'typing') { to(msg.to, { type: 'typing', from: id }) }
    else if (msg.type === 'read') { to(msg.to, { type: 'read', from: id, msgId: msg.msgId }) }
  ...
  ws.on('close', () => {
    if (id && users[id] === ws) {
      delete users[id]
      Object.keys(users).forEach(uid => to(uid, { type: 'user_offline', userId: id }))
    }
  })
  ws.on('error', () => {})
})

// Cleanup old pending messages (older than 7 days)
setInterval(() => {
  const week = 7 * 24 * 60 * 60 * 1000, now = Date.now()
  Object.keys(pending).forEach(uid => {
    pending[uid] = pending[uid].filter(m => (now - (m.ts || 0)) < week)
    if (!pending[uid].length) delete pending[uid]
  })
  Object.keys(friendReqs).forEach(uid => {
    friendReqs[uid] = friendReqs[uid].filter(r => (now - r.ts) < week)
    if (!friendReqs[uid].length) delete friendReqs[uid]
  })
}, 3600000)

server.listen(PORT, () => console.log(`Freeisle Relay on port ${PORT} — no database, pure relay`))
