...
// ── INIT ──
async function init() {
  config = await window.island.getConfig()
...
function startApp() {
  ...
  setTimeout(() => notify('🏝️ Island', 'Welcome back! Your vault is secure.', 'ns'), 1200)
}
...
function updateSetupId() {
  ...
  if (uname.length < 2) {
    preview.textContent = 'type username above...'
  } else {
    preview.textContent = `${uname}#${num}`
  }
}
...
async function finishSetup() {
  ...
  config = await window.island.saveConfig({ userId, username: uname, firstRun: false })
  document.getElementById('setup-page').classList.remove('on')
  startApp()
  notify('🏝️ Welcome!', `Your Island ID is ${userId}`, 'ns')
}
...
async function checkPin() {
  if (!pinEntry) return
  const r = await window.island.verifyPin(pinEntry)
...
function applyTheme(theme) {
  ...
  document.querySelectorAll('.td').forEach((d, i) => {
    d.classList.toggle('on', ['dark-ocean', 'jungle', 'sandy', 'purple', 'midnight'][i] === theme)
  })
}
async function setTheme(theme) {
  applyTheme(theme)
  config = await window.island.saveConfig({ theme })
...
function copyId() {
  ...
  notify('ID Copied', 'Your Island ID is in your clipboard', 'ns')
}
function copyBtc() {
...
async function copyVaultPath() {
  const path = await window.island.getVaultPath()
  navigator.clipboard.writeText(path)
  toast('Vault path copied', 'ts')
}
async function openVault() {
  const path = await window.island.getVaultPath()
  window.island.openFile(path)
}
...
async function changeUsername() {
  ...
  const newId = `${val}#${num}`
  config = await window.island.saveConfig({ userId: newId, username: val })
  updateMyId()
...
// ── CHAT STORAGE ──
function saveChatStorage() { localStorage.setItem('island-chats-v3', JSON.stringify(chats)) }
function loadChatsFromStorage() {
  const raw = localStorage.getItem('island-chats-v3') || localStorage.getItem('island-chats-v2') || localStorage.getItem('island-chats')
  if (raw) { try { chats = JSON.parse(raw) } catch(e) { chats = {} } }
  if (!Object.keys(chats).length) {
    chats['demo'] = {
      id: 'islandteam#0001', name: 'Island Team',
      messages: [
        { id: '1', from: 'islandteam#0001', text: 'Welcome to Island! 🏝️', time: Date.now() - 3600000, type: 'text' },
        { id: '2', from: 'me', text: 'This is amazing!', time: Date.now() - 3000000, type: 'text' },
        { id: '3', from: 'islandteam#0001', text: 'Your data stays only here. Always. 🔒', time: Date.now() - 1800000, type: 'text' },
      ],
...
function renderChatList(filter = '') {
  ...
  if (!entries.length) { el.innerHTML = '<div style="padding:24px;text-align:center;font-size:12px;color:var(--text3)">No chats yet.<br>Add a friend by their ID.</div>'; return }
  el.innerHTML = entries.map(([key, c]) => {
    ...
    return `<div class="ci ${activeChatId === key ? 'on' : ''}" onclick="openChat('${key}')">
      ...
      <div class="cinf">
        <div class="cnm">${c.name}${c.isGroup ? '<span class="grpbdg">GROUP</span>' : ''}</div>
        <div class="cprev">${(preview + '').substring(0, 40)}</div>
      </div>
      <div class="cmeta">
...
function openChat(key) {
  activeChatId = key; const c = chats[key]; c.unread = 0
  ...
  renderMessages(); renderChatList(); saveChatStorage()
}
...
function renderMessages() {
  ...
  if (!c.messages.length) { el.innerHTML = `<div class="sysmsg">Start of your conversation with ${c.name}. End-to-end encrypted 🔒</div>`; return }
  el.innerHTML = c.messages.map(m => {
    ...
    return `<div class="msg ${mine ? 'sent' : 'recv'} ${m.selfDestruct ? 'sdmsg' : ''}" data-id="${m.id}">
      ${!mine && c.isGroup ? `<div class="msndr">${m.from}</div>` : ''}
      ${bubble}
...
function sendMsg() {
  ...
  const msg = { id: Date.now().toString(), from: 'me', text, time: Date.now(), type: 'text', replyTo: replyTo ? replyTo.substring(0, 60) : null, selfDestruct }
  c.messages.push(msg)
  input.value = ''; input.style.height = 'auto'
  cancelReply(); renderMessages(); renderChatList(); saveChatStorage()
  if (selfDestruct) {
    setTimeout(() => {
      if (chats[activeChatId]) { chats[activeChatId].messages = chats[activeChatId].messages.filter(m => m.id !== msg.id); renderMessages(); saveChatStorage() }
    }, 10000)
  }
  sendOverNetwork(activeChatId, msg)
}
...
function clearChat() {
  if (!activeChatId) return
  chats[activeChatId].messages = []; renderMessages(); saveChatStorage(); toast('Chat cleared', 'ts')
}
...
function addChat() {
  ...
  renderChatList(); openChat(key); saveChatStorage()
  toast(`Connected to ${id}`, 'ts')
  notify('New Chat', `Connected to ${id}`, 'nm')
}
...
async function savePin(type) {
  ...
  if (type === 'fake') config = await window.island.saveConfig({ fakePin: val })
  else config = await window.island.saveConfig({ pin: val })
...
async function doWipe() {
  ...
  await window.island.emergencyWipe()
}
...
function saveVoiceMsg() {
  ...
  renderMessages(); saveChatStorage(); toast('Voice message sent', 'ts')
}

async function sendFileInChat() {
  if (!activeChatId) return
  const paths = await window.island.selectFiles()
  ...
  renderMessages(); saveChatStorage(); toast(`${paths.length} file(s) shared`, 'ts')
}

// ── DISCOVER PAGE ──
const knownUsers = [] // In real P2P this would come from DHT
function searchDiscover(query) {
  ...
  // Simulate discovery - in real app this queries DHT network
  const results = knownUsers.filter(u => u.name.toLowerCase().includes(q) || u.id.toLowerCase().includes(q))
  // Also show if they typed a valid ID directly
  if (q.includes('#')) {
    const exists = results.find(r => r.id.toLowerCase() === q)
    if (!exists) results.push({ id: q, name: q.split('#')[0], online: false, simulated: true })
  }
  if (!results.length) {
    el.innerHTML = `<div style="color:var(--text3);font-size:13px;padding:20px 0;">No users found for "${query}".<br><br>If you know their ID, you can add them directly from the Chat page using the + button.</div>`
    return
  }
  el.innerHTML = results.map(u => `
    <div class="disc-user">
      ...
      <div class="disc-info">
        <div class="disc-name">${u.name}</div>
        <div class="disc-id">${u.id}</div>
...
function setupDropZone() {
  ...
  z.addEventListener('drop', async e => {
    ...
    for (const f of files) {
      ...
      await window.island.saveFile({ name: f.name, buffer: Array.from(new Uint8Array(buf)) })
    }
    loadFiles()
    toast(`${files.length} file(s) added`, 'ts')
  })
}
...
async function pickFiles() {
  ...
  for (const srcPath of paths) {
    try {
      await window.island.copyFileToVault({ srcPath, dest: 'files' })
      added++
    } catch(e) {
      // fallback: read and write
      toast('Copying files...', 'ts')
    }
  }
  loadFiles()
...
async function loadFiles() {
  const files = await window.island.getFiles()
  ...
  el.innerHTML = files.map(f => `
    <div class="fi">
      ...
      <div class="fif"><div class="fin">${f.name}</div><div class="fis">${fmtSize(f.size)} · ${new Date(f.modified).toLocaleDateString()}</div></div>
      <div class="fac">
        <button class="ib" onclick="window.island.openFile('${f.path.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}')" title="Open"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></button>
        <button class="ib" onclick="delFile('${f.name}')" title="Delete"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg></button>
      </div>
    </div>`).join('')
}

async function delFile(name) {
  await window.island.deleteFile(name); loadFiles(); toast('File deleted', 'ts')
}

// ── GALLERY (WORKING) ──
async function pickGallery() {
  const paths = await window.island.selectFiles({ images: true })
  if (!paths || !paths.length) return
  let added = 0
  for (const srcPath of paths) {
    try {
      await window.island.copyFileToVault({ srcPath, dest: 'gallery' })
      added++
    } catch(e) {}
  }
  loadGallery()
...
async function loadGallery() {
  const items = await window.island.getGallery()
  ...
  grid.innerHTML = items.map(item => {
    ...
    return `<div class="gi" onclick="window.island.openFile('${item.path.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}')" title="${item.name}">
      ${isVid
        ? `<video src="file:///${filePath}" muted preload="metadata"></video>`
        : `<img src="file:///${filePath}" alt="${item.name}" loading="lazy" onerror="this.style.display='none'">`
      }
      <div class="giov"><span>${item.name}</span></div>
    </div>`
  }).join('')
}
...
async function loadNotes() {
  const notes = await window.island.getNotes()
  const el = document.getElementById('nitems'); if (!el) return
  if (!notes.length) { el.innerHTML = '<div style="padding:16px;font-size:12px;color:var(--text3)">No notes yet. Click + to create one.</div>'; return }
  el.innerHTML = notes.map(n => `
    <div class="ni ${currentNoteId === n.id ? 'on' : ''}" onclick="openNote('${n.id}','${encodeURIComponent(n.title || '')}','${encodeURIComponent(n.content || '')}')">
      ...
      <div class="np">${(n.content || '').substring(0, 55)}</div>
...
function autoSave() {
  ...
  noteTimer = setTimeout(async () => {
    ...
    await window.island.saveNote({ id: currentNoteId, title, content })
    loadNotes()
  }, 700)
}
...
async function loadStorage() {
  const stats = await window.island.getDiskStats()
  const sg = document.getElementById('sgrid'); if (!sg) return
  sg.innerHTML = [
    { l: 'Vault Size', v: fmtSize(stats.vaultSize || 0), s: 'Total Island data' },
    { l: 'Free RAM', v: fmtSize(stats.freeMem || 0), s: 'Available memory' },
    { l: 'Messages', v: fmtSize(stats.messagesSize || 0), s: 'Chat history' },
  ...
  const bars = [
    { l: 'Messages', s: stats.messagesSize || 0, c: '#4f9eff' },
...
async function scanParts() {
  const parts = await window.island.detectDualBoot()
  const el = document.getElementById('partslist'); if (!el) return
  if (!parts.length) {
    el.innerHTML = '<div style="color:var(--text3);font-size:13px;padding:12px 0;">No Windows/exFAT partitions found.<br><span style="font-size:11px;">This works best on Kali Linux with dual boot installed.</span></div>'
    return
  }
  el.innerHTML = parts.map(p => `
    <div class="pcard">
      <div style="font-size:24px;">💿</div>
      <div style="flex:1;"><div class="pcnm">/dev/${p.name}</div><div class="pcmt">${p.type} · ${p.size}${p.mount ? ' · Mounted at ' + p.mount : ''}</div></div>
      <button class="btn btn-p" onclick="mountPart('${p.name}')">${p.mount ? 'Browse' : 'Mount'}</button>
    </div>`).join('')
}
async function mountPart(name) {
  toast('Mounting...', 'tw')
  const r = await window.island.mountPartition(name)
  if (r.ok) { toast(`Mounted at ${r.path}`, 'ts'); notify('Dual Bridge', 'Partition mounted successfully', 'ns') }
  else toast('Failed: ' + r.msg, 'te')
}

// ── NETWORK ──
function setupNetwork() { /* P2P relay - future implementation */ }
function sendOverNetwork(chatId, msg) {
  if (chatId === 'demo' && Math.random() > 0.4) {
    const replies = ['Got it! 🔒', 'Only we can read this.', 'Island keeps it private.', 'No servers here. 🏝️', 'Fully encrypted.']
    setTimeout(() => {
      const c = chats['demo']; if (!c) return
      const reply = { id: Date.now().toString(), from: c.id, text: replies[Math.floor(Math.random() * replies.length)], time: Date.now(), type: 'text' }
      c.messages.push(reply)
      if (activeChatId !== 'demo') { c.unread = (c.unread || 0) + 1; document.getElementById('cbdg').style.display = 'block'; notify('New message', reply.text.substring(0, 50), 'nm') }
      else renderMessages()
      renderChatList(); saveChatStorage()
...
function notify(title, text, type = 'nm') {
  ...
  el.innerHTML = `<div class="nic">${icons[type] || '🔔'}</div><div class="nib"><div class="nit">${title}</div><div class="nitx">${text}</div><div class="nitm">Just now</div></div>`
...
function fileIcon(name) {
  ...
  return { pdf: '📄', doc: '📝', docx: '📝', xls: '📊', xlsx: '📊', ppt: '📊', pptx: '📊', zip: '🗜️', rar: '🗜️', '7z': '🗜️', jpg: '🖼️', jpeg: '🖼️', png: '🖼️', gif: '🖼️', webp: '🖼️', mp4: '🎬', mkv: '🎬', avi: '🎬', mp3: '🎵', wav: '🎵', flac: '🎵', exe: '⚙️', sh: '⚙️', py: '🐍', js: '📜', html: '🌐', css: '🎨', txt: '📃', json: '📋' }[ext] || '📄'
}
...
async function checkVaultStatus() {
  const info = await window.island.getVaultInfo()
  // Show vault status in titlebar
  const slogan = document.querySelector('#tb .aslogan')
  if (slogan) {
    if (info.isVirtualDisk) {
      slogan.textContent = `Your island. Your rules. Your freedom. · 💾 Virtual disk active`
      slogan.style.color = '#22d98a'
    } else {
      slogan.textContent = 'Your island. Your rules. Your freedom. · Creating virtual disk...'
    }
  }
}

window.island.onVaultMounted(info => {
  const slogan = document.querySelector('#tb .aslogan')
  if (slogan) {
    slogan.textContent = `Your island. Your rules. Your freedom. · 💾 Island disk mounted at ${info.path}`
    slogan.style.color = '#22d98a'
  }
  notify('💾 Virtual Disk', `Island disk created and mounted!`, 'ns')
  toast('Island virtual disk is ready! 💾', 'ts')
})
...
async function pickConvertFile() {
  ...
  if (!formats) { toast(`Format .${ext} not supported for conversion`, 'te'); return }
  convFilePath = filePath
  convSelectedFormat = null
  ...
  drop.style.border = '2px solid var(--success)'
  drop.querySelector('h3').textContent = name
  // render format buttons
  document.getElementById('conv-formats').innerHTML = formats.map(f => `
    <button onclick="selectConvFormat('${f}',this)" style="padding:10px 20px;border-radius:10px;background:var(--bg3);border:2px solid var(--border);color:var(--text2);font-family:var(--font);font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;">
      .${f.toUpperCase()}
    </button>`).join('')
}
...
function selectConvFormat(fmt, btn) {
  ...
  document.querySelectorAll('#conv-formats button').forEach(b => {
...
async function doConvert() {
  ...
  btn.disabled = true
  const result = await window.island.convertFile({ srcPath: convFilePath, outputFormat: convSelectedFormat })
  ...
  if (result.ok) {
    ...
    document.getElementById('conv-open-btn').onclick = () => window.island.openFile(result.path)
    toast('Conversion complete! ✅', 'ts')
    notify('🔄 Converter', `${result.name} saved to your vault`, 'ns')
  } else {
    toast('Conversion failed: ' + result.msg, 'te')
  }
}
...
function clearConverter() {
  ...
  drop.querySelector('h3').textContent = 'Drop a file here or click to browse'
}

// Setup converter drop zone
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (convDrop) {
    ...
    convDrop.addEventListener('dragleave', () => { convDrop.style.borderColor = 'var(--border2)' })
    convDrop.addEventListener('drop', async e => {
      e.preventDefault()
      convDrop.style.borderColor = 'var(--border2)'
      const file = e.dataTransfer.files[0]
      if (!file) return
      // Save temp and use path
      const ext = file.name.split('.').pop().toLowerCase()
      if (!FORMAT_MAP[ext]) { toast(`Format .${ext} not supported`, 'te'); return }
      // For dropped files we need to read them
      const buf = await file.arrayBuffer()
      await window.island.saveFile({ name: file.name, buffer: Array.from(new Uint8Array(buf)) })
      const files = await window.island.getFiles()
      ...
      if (saved) {
        ...
        const formats = FORMAT_MAP[ext]
        document.getElementById('conv-formats').innerHTML = formats.map(f => `
          <button onclick="selectConvFormat('${f}',this)" style="padding:10px 20px;border-radius:10px;background:var(--bg3);border:2px solid var(--border);color:var(--text2);font-family:var(--font);font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;">.${f.toUpperCase()}</button>`).join('')
        document.getElementById('conv-result').style.display = 'none'
      }
    })
  }
...
// Check vault on start
setTimeout(checkVaultStatus, 3000)
